...
def render_async(self, *args, **kwargs):
    ...
    try:
        return await
        concat_async(self.root_render_func(ctx))
    except Exception:
        exc_info = sys.exc_info()
...
def wrap_block_reference_call(original_call):
    ...
    def async_call(self):
        rv = await
        concat_async(self._stack[self._depth](self._context))
...
def make_module_async(self, vars=None, shared=False, locals=None):
    ...
    body_stream = []
    async
    for item in self.root_render_func(context):
